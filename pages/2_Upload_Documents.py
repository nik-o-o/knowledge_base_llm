...
st.set_page_config(page_title="Knowledge Base - Upload Documents", page_icon="📂")
...
# Sidebar header
st.sidebar.markdown(
    "<h2 style='text-align: center;'>Knowledge Base</h2>", unsafe_allow_html=True
)
st.sidebar.markdown(
...
def render_upload_page() -> None:
    ...
    if st.session_state["documents"]:
        ...
        delete_all_button = st.button(
            "Delete all",
            key="delete_all",
            help="Delete all uploaded documents",
            type="primary",
        )
        if delete_all_button:
            ...
            # Clear documents from OpenSearch and session state
            for doc in st.session_state["documents"]:
        ...
        with st.expander("Manage Uploaded Documents", expanded=True):
            # Display individual documents with delete buttons
